import { SCHOOL_GLYPHS, SCHOOL_UPEM } from './schoolGlyphs';
...
/**
 * Lays out tracing text from the bundled school-print outlines (single-storey "a").
 *
 ...
 *
 * The caller draws: <G transform={`translate(x baseline) scale(scale)`}> with one
 * <Path d={g.d} transform={`translate(${g.x} 0)`} /> per glyph.
 */
export function layoutSchoolText(text: string, maxWidth: number, maxFontSize: number): SchoolTextLayout {
  const items = [...text].map((ch) => SCHOOL_GLYPHS[ch] ?? { d: '', a: SPACE_UNITS });
